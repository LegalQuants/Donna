...
describe('buildScheduleBody', () => {
	...
	it('fails when the source or cron is missing', () => {
		expect(
			buildScheduleBody(fd({ source_mode: 'playbook', cron_expr: '0 9 * * *' }), 'create').ok
		).toBe(false);
		expect(buildScheduleBody(fd({ source_mode: 'playbook', playbook_id: 'p1' }), 'create').ok).toBe(
			false
		);
	});
	...
	it('update: an absent project_id field also maps to null', () => {
		const out = buildScheduleBody(
			fd({ source_mode: 'playbook', playbook_id: 'p1', cron_expr: '0 9 * * *' }),
			'update'
		);
		expect(out.ok && out.body.project_id).toBeNull();
	});
