...
<script lang="ts">
	...
	// Shared enhance callback: always run the default update (optimistic UI),
	// and when the server returns a failure (e.g. 404 row no longer exists)
	// invalidateAll so the page refreshes and the stale row disappears.
...
<div class="flex flex-col gap-2 rounded-mlq-control border border-mlq-subtle p-3">
	...
	{#if actionable}
		...
		<div class="flex flex-wrap items-center gap-2">
			<!-- Keep -->
			<form method="POST" action="?/keep" use:enhance={submitFn} class="shrink-0">
			...
			<!-- Dismiss -->
			<form method="POST" action="?/dismiss" use:enhance={submitFn} class="shrink-0">
				<input type="hidden" name="id" value={memory.id} />
	...
	{:else}
		...
		{#if confirmingDelete}
			<div class="flex items-center gap-2">
				...
				<form method="POST" action="?/delete" use:enhance={submitFn} class="shrink-0">
					<input type="hidden" name="id" value={memory.id} />
