...
<div class="rounded-mlq-control border border-mlq-subtle p-4">
  <div class="flex flex-wrap items-center gap-2">
    ...
    {#if session.cost_cap_reached}<span class="text-xs text-mlq-caveats">cost cap reached</span>{/if}
  ...
  {#if session.error}
    <p class="mt-2 rounded-mlq-control bg-mlq-error/10 p-2 text-xs text-mlq-error">Error: {session.error}</p>
  {/if}
</div>
