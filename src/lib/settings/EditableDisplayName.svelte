<script lang="ts">
	...
	const canSave = $derived(
		trimmed.length > 0 && trimmed.length <= MAX_DISPLAY_NAME_LENGTH && trimmed !== (name ?? '')
	);
...
<div class="flex items-start justify-between px-4 py-2">
	...
	<dd class="m-0 flex flex-col items-end gap-1 text-mlq-text">
		{#if editing}
			...
			>
				...
				<button
					type="submit"
					disabled={!canSave}
					class="rounded-mlq-control bg-mlq-strong px-2.5 py-1 text-xs text-white disabled:cursor-not-allowed disabled:opacity-50"
					>Save</button
				>
				<button
					type="button"
		...
		{:else}
			<span class="flex items-center gap-2">
				...
				<button
					type="button"
					onclick={startEdit}
					class="rounded-mlq-control border border-mlq-subtle px-2 py-0.5 text-xs text-mlq-text hover:bg-mlq-subtle/50"
					>Edit</button
				>
			</span>
		{/if}
		{#if msg}<p
				role="status"
				aria-live="polite"
				class="text-xs {msgIsError ? 'text-mlq-error' : 'text-mlq-muted'}"
			>
				{msg}
			</p>{/if}
	</dd>
