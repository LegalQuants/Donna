...
describe('editorDraft', () => {
	...
	it('isPositionValid requires issue + standard_language + severity', () => {
		expect(
			isPositionValid({ issue: 'I', standard_language: 'L', severity_if_missing: 'high' })
		).toBe(true);
		expect(
			isPositionValid({ issue: ' ', standard_language: 'L', severity_if_missing: 'high' })
		).toBe(false);
		expect(
			isPositionValid({ issue: 'I', standard_language: '', severity_if_missing: 'high' })
		).toBe(false);
	});
