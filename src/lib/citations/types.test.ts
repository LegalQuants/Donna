...
describe('citeState', () => {
	...
	it('verified: undefined → unverified', () => {
		expect(citeState(base({ verified: undefined }))).toBe('unverified');
	});
...
describe('tooltipFor', () => {
	...
	it('tolerant_match tooltip', () => {
		expect(
			tooltipFor(base({ verification_method: 'tolerant_match', verification_confidence: 1 }))
		).toBe('Verified — matches source (normalized) (100%)');
	});
	it('ensemble_strict tooltip', () => {
		expect(
			tooltipFor(
				base({ verification_method: 'ensemble_strict', verification_confidence: 1, partial: false })
			)
