<script lang="ts">
  ...
  let { open, skill, onclose }: { open: boolean; skill: SkillSummary | null; onclose: () => void } = $props();
...
{#if open && skill}
  ...
  <div role="dialog" aria-modal="true" aria-label="Fork skill"
    ...
    <p class="mb-3 text-xs text-mlq-muted">Creates an editable copy in your skills. Pick an id (slug) — this can't be changed later.</p>
    <form
      method="POST"
      action="?/fork"
    ...
    >
      ...
      <label class="block text-xs text-mlq-muted">
        Slug
        <input name="new_name" type="text" required bind:value={slug}
