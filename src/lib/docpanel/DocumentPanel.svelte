...
>
	...
	{#if docPanel.activeTab && docPanel.activeTab.mime === 'application/pdf' && docPanel.activeTab.status !== 'error'}
		...
		<div
			class="flex items-center gap-2 border-b px-3 py-1.5 text-[11px] {tab.highlightStatus ===
			'miss'
				? 'border-mlq-caveats/40 bg-mlq-caveats/10'
				: 'border-mlq-subtle bg-mlq-surface-alt'}"
		>
			{#if tab.page}
			...
			{#if tab.highlightStatus === 'miss'}
				<span class="line-clamp-2 min-w-0 text-mlq-text"
					>Cited passage on this page — couldn't pinpoint the exact span. <span
						class="text-mlq-muted italic">"{tab.quote}"</span
					></span
				>
			{:else}
				<span class="truncate text-mlq-muted italic">"{tab.quote}"</span>
