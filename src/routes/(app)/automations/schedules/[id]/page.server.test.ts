...
const sched = { id: 's1', cron_expr: '0 9 * * 1', playbook_id: 'p1', skill_ref: null, target_kb_id: 'kb1', project_id: null, max_cost_usd: null, enabled: true, name: 'Weekly', next_run_at: null, last_run_at: null };
...
describe('/automations/schedules/[id] update', () => {
  ...
  it('keeps the generic message for a schedule-not-found 404', async () => {
    lqFetch.mockResolvedValueOnce(new Response(JSON.stringify({ detail: 'autonomous schedule not found' }), { status: 404 }));
