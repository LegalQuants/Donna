...
describe('/automations/schedules page', () => {
  it('shows the opt-in gate when autonomous is off', () => {
    render(Page, { props: { data: { autonomousEnabled: false, unread: 0, schedules: [], ...libs }, form: null } as never });
  ...
  it('shows the New schedule control and the list when opted in', () => {
    const schedule = { id: 's1', name: 'Weekly', cron_expr: '0 9 * * 1', playbook_id: 'p1', skill_ref: null, target_kb_id: null, project_id: null, max_cost_usd: null, enabled: true, next_run_at: null, last_run_at: null };
    render(Page, { props: { data: { autonomousEnabled: true, unread: 0, schedules: [schedule], playbookItems: [{ value: 'p1', label: 'NDA' }], skillItems: [], kbs: [], matters: [] }, form: null } as never });
