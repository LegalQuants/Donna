...
export const actions: Actions = {
	...
	detachFile: async (event) => {
		...
		const res = await lqFetch(event, `/api/v1/projects/${event.params.id}/files/${file_id}`, {
			method: 'DELETE'
		});
	...
	detachSkill: async (event) => {
		...
		const skill_name = String(data.get('skill_name') ?? '');
		if (!skill_name) return fail(400, { error: 'Missing skill_name.' });
		const res = await lqFetch(event, `/api/v1/projects/${event.params.id}/skills/${skill_name}`, {
			method: 'DELETE'
		});
