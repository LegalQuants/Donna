...
describe('/settings/models load', () => {
  it('admin: fetches models + admin aliases, builds categories from the alias entries', async () => {
    ...
    const out = (await load(ev(true))) as { isAdmin: boolean; categories: { name: string; currentTargetId: string | null }[]; targets: unknown[]; localModels: unknown[]; modelsError: boolean; providerKeys: unknown[] | null };
