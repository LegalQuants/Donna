...
const MODELS = {
	...
	data: [
		...
		},
		{
			...
			lq_ai_resolves_to: 'ollama/llama3'
		}
	]
};
const CFG = {
	allowed_tiers_global: [1, 2, 3, 4],
	default_minimum_tier: 1,
	privileged_minimum_tier: 4,
	warn_on_tiers: []
};
...
// `load`'s SvelteKit return type includes `void`; narrow to the concrete shape for assertions.
type LoadResult = {
	rows: Array<{ id: string; where: string }>;
	modelsError: boolean;
	tierConfig: unknown;
};
const callLoad = async (): Promise<LoadResult> => (await load(event)) as LoadResult;

beforeEach(() => lqFetch.mockReset());

function route(map: Record<string, Response>) {
	lqFetch.mockImplementation((_e: unknown, path: string) =>
		Promise.resolve(map[path] ?? new Response(null, { status: 500 }))
	);
}
...
describe('trust page load', () => {
	it('returns matrix rows + tierConfig when both fetches succeed', async () => {
		...
		expect(r.rows.map((x) => [x.id, x.where])).toEqual([
			['smart', 'Cloud'],
			['local', 'Local']
		]);
	...
	it('sets modelsError + empty rows when /models fails, still returns tierConfig', async () => {
		route({
			'/api/v1/models': new Response(null, { status: 500 }),
			'/api/v1/inference/tier-config': ok(CFG)
		});
		const r = await callLoad();
	...
	it('returns tierConfig null when tier-config fails, still returns rows', async () => {
		route({
			'/api/v1/models': ok(MODELS),
			'/api/v1/inference/tier-config': new Response(null, { status: 500 })
		});
		const r = await callLoad();
