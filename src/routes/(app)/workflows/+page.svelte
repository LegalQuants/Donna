<script lang="ts">
	...
	const cards = [
		...
		},
		{
			...
			icon: Bot,
			name: 'Automations',
			desc: 'Sessions Donna runs on its own — see exactly what the agent did, what it cost, and why it stopped.'
		}
	];
</script>
...
<div class="mx-auto max-w-3xl px-4 py-6">
	...
	<div data-testid="workflows-cards" class="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
		{#each cards as c (c.href)}
			<a
				href={c.href}
				class="block rounded-mlq-control border border-mlq-subtle p-4 hover:bg-mlq-subtle/40 focus-visible:ring-2 focus-visible:ring-mlq-workflow focus-visible:outline-none"
			>
