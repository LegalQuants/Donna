	...
	it('loads active user skills + built-ins', async () => {
		lqFetch
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify([
						{ name: 'cr', title: 'Contract Review', version: '1', scope: 'builtin' }
					]),
					{ status: 200 }
				)
			);
...
describe('/skills ?/create', () => {
	...
	it('omits slash_alias when blank', async () => {
		...
		await expect(
			actions.create(formEv({ display_name: 'NDA', slug: 'nda', description: 'x', body: 'B' }))
	...
	it('maps 409 to an inline slug error', async () => {
		...
		const r = await actions.create(
			formEv({ display_name: 'NDA', slug: 'nda', description: 'x', body: 'B' })
		);
	...
	it('maps a slash_alias 422 to an inline slash_alias error', async () => {
		lqFetch.mockResolvedValueOnce(
			new Response(
				JSON.stringify({ detail: "slash_alias '/nda' is already used by another of your skills." }),
				{ status: 422 }
			)
		);
		const r = await actions.create(
			formEv({ display_name: 'NDA', slug: 'nda', description: 'x', body: 'B', slash_alias: '/nda' })
		);
...
describe('/skills ?/fork', () => {
	...
	it('maps 409 to a friendly already-forked error', async () => {
		...
		expect(r).toMatchObject({
			status: 409,
			data: { error: 'You already have a skill with that id — pick a different slug.' }
		});
	});
	...
	it('maps a 422 (invalid slug) to a friendly error', async () => {
		...
		expect(r).toMatchObject({
			status: 422,
			data: { error: 'That id isn’t valid — use lowercase letters, numbers, and dashes.' }
		});
	});
