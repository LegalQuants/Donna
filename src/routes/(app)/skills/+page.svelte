<script lang="ts">
	...
	const filteredBuiltins = $derived(
		builtinQ.trim()
			? data.builtins.filter((s) =>
					(s.title + ' ' + (s.description ?? ''))
						.toLowerCase()
						.includes(builtinQ.trim().toLowerCase())
				)
			: data.builtins
	);
</script>
...
<div class="mx-auto max-w-3xl px-4 py-6">
	...
	<h2 class="mb-2 text-xs font-medium tracking-wide text-mlq-muted uppercase">Your skills</h2>
				...
				{#each filteredBuiltins as b (b.name)}
					<li class="flex items-center gap-3 border-b border-mlq-subtle px-3 py-2 last:border-b-0">
						<span class="min-w-0 flex-1">
							...
							{#if b.description}<span class="block truncate text-xs text-mlq-muted"
									>{b.description}</span
								>{/if}
						</span>
