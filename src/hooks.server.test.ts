...
const mockUser = (over: Record<string, unknown> = {}) =>
	lqFetch.mockResolvedValue(
		new Response(JSON.stringify({ id: 'u1', must_change_password: false, ...over }), {
			status: 200
		})
	);
...
describe('handle hook auth routing', () => {
	it('redirects an authed user away from /login to /', async () => {
		mockUser();
		await expect(
			handle({ event: ev({ routeId: '/(auth)/login', path: '/login', authed: true }), resolve })
	...
	it('lets an authed user reach /change-password voluntarily', async () => {
		mockUser();
		await handle({
			event: ev({ routeId: '/(auth)/change-password', path: '/change-password', authed: true }),
			resolve
		});
	...
	it('forces password change on an app route when required', async () => {
		...
		await expect(
			handle({
				event: ev({ routeId: '/(app)/settings/account', path: '/settings/account', authed: true }),
				resolve
			})
		).rejects.toMatchObject({ status: 303, location: '/change-password' });
	});

	it('redirects an unauthenticated user away from app routes', async () => {
		await expect(
			handle({
				event: ev({ routeId: '/(app)/settings/account', path: '/settings/account', authed: false }),
				resolve
			})
