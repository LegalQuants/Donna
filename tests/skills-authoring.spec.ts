...
test('skills authoring — create (with description), edit, fork built-in (default slug), archive', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByRole('heading', { name: 'Skills', exact: true, level: 1 })).toBeVisible({ timeout: 10_000 });

    // Assert the "Built-in skills" section heading is visible.
    await expect(page.getByRole('heading', { name: 'Built-in skills', exact: true })).toBeVisible({ timeout: 10_000 });
    ...
    await page.waitForURL('**/skills', { timeout: 10_000 });
    await expect(page.locator(`a[href="/skills/${forkedSkillId}"]`)).toHaveCount(0, { timeout: 10_000 });
