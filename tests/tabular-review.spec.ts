...
test('tabular: build at /tabular/new — upload, column, preview, run, grid, export', async ({
	page
}) => {
...
test('tabular: /tabular is the history index — lists runs, resumes one, opens New review', async ({
	page
}) => {
	test.setTimeout(300_000);
	await login(page);
	...
	await page.goto('/tabular');
	await page
		.getByRole('link', { name: /new review/i })
		.first()
		.click();
...
test('tabular: a cell citation opens the cited source in the doc panel', async ({ page }) => {
	...
	await expect(page.getByRole('complementary', { name: /document panel/i })).toBeVisible({
		timeout: 30_000
	});
});
...
test('tabular: run a built-in table skill — its resolved columns render in the grid', async ({
	page
}) => {
	test.setTimeout(300_000);
	await login(page);
