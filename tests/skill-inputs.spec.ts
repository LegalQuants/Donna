...
const INPUT_TEXT_VALUE = 'The parties agree that all obligations hereunder shall be subject to confidentiality requirements as defined in Section 4.2 of the Master Agreement.';
...
test('composer: attach a skill, fill its required inputs, send button gates correctly', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByTestId(`skill-result-${SKILL_RESULT_SLUG}`)).toBeVisible({ timeout: 10_000 });
