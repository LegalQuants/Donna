...
**Goal.** Deliver the transparency thesis of the autonomous layer — *"see exactly what the agent did, what it cost, and why it stopped"* — as a clean 4th segment in Donna's existing Workflows IA, plus the report-back inbox. Both slices are **read-only** and require **no `autonomous_enabled` opt-in**, so they ship with zero upstream dependencies.
