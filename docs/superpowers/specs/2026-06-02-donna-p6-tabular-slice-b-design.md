...
**Slice B** adds the _around-the-run_ surface: an **executions history list** as the new `/tabular`
landing, **resume** (open a past run from the list), and — _if the backend gains it_ — **cell→source
...
1. **IA:** `/tabular` becomes the **executions history index** with a _New review_ entry; the builder
...
document-specific message — e.g. _"One or more selected documents couldn't be found or isn't accessible.
Re-check your document selection."_ Keep 503/504 passthrough and the generic 502 for other failures.
...
- The doc panel `docPanel.open(Citation)` needs `source_file_id` (a **`files.id`**), `source_page`,
  ...
  - `value` + `confidence` + `document_id` — and that `document_id` is a **`documents.id`**, _not_ a
...
- This matches DE-309: the tabular citation is _"display-only and never resolves."_
...
**Pin-gated task (last in the plan):** _if_ the upstream SHA lands during this slice → bump
`vendor/lq-ai` + regenerate `backend.d.ts`, then make each citation in `CellDetail` clickable →
`docPanel.open({ source_file_id, source_page, source_text })` (~10 lines + a test + a live e2e). _If
not_ → leave counts-only; this task ships as a fast follow (P6-B.1) when the SHA arrives. The rest of
