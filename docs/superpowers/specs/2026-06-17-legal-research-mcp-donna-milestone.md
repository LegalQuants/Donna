...
*only* what Donna (the frontend) builds. All legal-AI logic — the gateway egress boundary, the
...
request** (§8) and relayed to the LQ-AI session for a backend refinement, *before* the shape is
