...
the layer _ran_, not that PII was found): "This request was processed by the anonymization
...
- **e2e (Playwright, live):**
  - _Receipts drawer:_ open the seeded (project-backed) chat → click **Receipts** →
  ...
  - _Anonymization indicator — live `true`:_ a spike confirmed we **can** produce a real
    ...
    phone + organization, e.g. _"…John Smith (john.smith@acme.com, 555-123-4567) of Acme
    Corporation…"_), then loads `/chats/{id}` in the UI and asserts the **"Anonymized"
    badge** is visible on the assistant turn.
  - _No false positive:_ on a non-PII turn (`anonymization_applied=false`) assert the
    badge is **absent**.
