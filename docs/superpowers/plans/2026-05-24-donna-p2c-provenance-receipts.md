	...
	switch (e.kind) {
		...
		case 'retrieval': {
			...
			return {
				...
				detail: bits.join(' · ') || 'retrieved context',
				tone: 'default'
			};
		}
		case 'inference':
		case 'error': {
			...
			if (e.kind === 'error' || d.refused === true) {
				return {
					label: 'Inference refused',
					detail: str(d, 'refusal_reason') ?? 'inference refused',
					tone: 'error',
					tier
				};
			}
		...
		case 'skill':
			return {
				label: 'Skill applied',
				detail: str(d, 'name') ?? str(d, 'skill') ?? str(d, 'skill_name') ?? 'skill',
				tone: 'default'
			};
		case 'audit':
...
describe('GET receipts', () => {
	it('proxies the receipts endpoint', async () => {
		lqFetch.mockResolvedValue(
			new Response(JSON.stringify([{ ts: 't', kind: 'message', detail: {} }]), { status: 200 })
		);
	...
	return new Response(res.body, {
		...
		headers: {
			...
			'content-disposition':
				...
				`attachment; filename="chat-${event.params.id}-receipts.jsonl"`,
...
describe('ReceiptEventRow', () => {
	...
	it('toggles the raw-detail expander', async () => {
		const { getByRole, queryByText, getByText } = render(ReceiptEventRow, {
			props: { event: inf }
		});
...
3f. **Make the existing P2b fetch-count assertions robust.** `loadAnonymization` now issues an _unconditional_ post-completion receipts fetch, so tests that asserted exact `fetch` call counts/indices must switch to URL-based assertions. Apply these three edits in `chatStream.svelte.test.ts`:
...
{#if message.anonymized === true}
	<span
		...
		title="Personal data was anonymized before this request left your environment"
	>
...
test('Receipts drawer opens and shows the provenance timeline', async ({ page }) => {
	...
	await expect(drawer.getByRole('link', { name: /export/i })).toHaveAttribute(
		'href',
		/receipts\/export\.jsonl$/
	);
...
test('Anonymized badge shows for a PII turn and is absent otherwise', async ({ page }) => {
	...
	const piiChat = await seedChat(
		tok,
		'Draft a one-line note that John Smith (john.smith@acme.com, 555-123-4567) of Acme Corporation received the documents.'
	);
