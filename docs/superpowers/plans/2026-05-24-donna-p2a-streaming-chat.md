...
.prose-mlq { font-family: var(--font-serif); color: var(--color-mlq-text); line-height: 1.65; }
.prose-mlq p { margin: 0 0 0.75rem; }
.prose-mlq h1, .prose-mlq h2, .prose-mlq h3 { color: var(--color-mlq-strong); font-weight: 500; margin: 1rem 0 0.5rem; }
.prose-mlq ul, .prose-mlq ol { margin: 0 0 0.75rem 1.25rem; }
.prose-mlq li { margin: 0.15rem 0; }
.prose-mlq a { color: var(--color-mlq-workflow); text-decoration: underline; }
.prose-mlq table { border-collapse: collapse; margin: 0.5rem 0; font-size: 0.9em; }
.prose-mlq th, .prose-mlq td { border: 1px solid var(--color-mlq-subtle); padding: 4px 8px; }
.prose-mlq blockquote { border-left: 3px solid var(--color-mlq-subtle); margin: 0 0 0.75rem; padding-left: 0.75rem; color: var(--color-mlq-muted); }
.prose-mlq code { background: var(--color-mlq-surface-alt); padding: 1px 4px; border-radius: 4px; font-size: 0.9em; }
```
...
describe('parseDataPayload', () => {
  ...
  it('parses a delta frame', () => {
    expect(parseDataPayload('{"type":"delta","delta":"hi","lq_ai_message_id":"m1","routed_inference_tier":3}'))
      .toMatchObject({ type: 'delta', delta: 'hi', routed_inference_tier: 3 });
  });
  it('maps the detail error envelope to an error frame', () => {
    expect(parseDataPayload('{"detail":{"code":"gateway_timeout","message":"timed out"}}'))
      .toEqual({ type: 'error', code: 'gateway_timeout', message: 'timed out' });
  });
...
describe('Message', () => {
  it('renders a user turn as a plain chip (no markdown block)', () => {
    const { container, getByText } = render(Message, { props: { message: { id: 'u1', role: 'user', content: 'hello there' } } });
  ...
  it('renders an assistant turn as markdown prose with the tier chip', () => {
    const { container, getByText } = render(Message, {
      props: { message: { id: 'a1', role: 'assistant', content: '**done**', routed_inference_tier: 3, status: 'done' } }
    });
  ...
  it('shows an error with a Retry button that calls onretry', async () => {
    ...
    const { getByRole } = render(Message, {
      props: { message: { id: 'a1', role: 'assistant', content: '', status: 'error', error: 'gateway timeout' }, onretry: () => (retried = true) }
    });
...
test('streams an assistant reply with a resolved tier, and it persists on reload', async ({ page }) => {
  await login(page);
