...
describe('buildScheduleBody', () => {
	...
	it('builds a skill body and honors enabled=false', () => {
		const out = buildScheduleBody(
			fd({ source_mode: 'skill', skill_ref: 'comms', cron_expr: '0 9 * * *', enabled: 'false' })
		);
		expect(out.ok && out.body).toEqual({
			cron_expr: '0 9 * * *',
			enabled: false,
	...
	it('fails when the source or cron is missing', () => {
		expect(buildScheduleBody(fd({ source_mode: 'playbook', cron_expr: '0 9 * * *' })).ok).toBe(
			false
		);
...
export type ScheduleBodyResult = { ok: true; body: Record<string, unknown> } | { ok: false };
...
describe('ScheduleForm', () => {
	...
	it('emits playbook_id + cron_expr + enabled hidden inputs', async () => {
		...
		expect((container.querySelector('input[name="cron_expr"]') as HTMLInputElement).value).toBe(
			'0 9 * * *'
		);
...
<div class="flex flex-col gap-4">
	<div>
		<label for="schedule-name" class="mb-1 block text-xs font-medium text-mlq-muted"
			>Name (optional)</label
		>
		<input
			id="schedule-name"
	...
	<div>
		<div class="mb-1 text-xs font-medium text-mlq-muted">
			Schedule <span class="text-mlq-error">*</span>
		</div>
	...
	<div>
		<label for="schedule-cost-cap" class="mb-1 block text-xs font-medium text-mlq-muted"
			>Cost cap (optional, USD)</label
		>
		<input
			id="schedule-cost-cap"
			type="number"
			min="0"
			step="0.01"
			inputmode="decimal"
...
describe('ScheduleRow', () => {
	...
	it('links to the edit page and exposes a delete form', () => {
		const { container } = render(ScheduleRow, {
			props: { schedule: { ...schedule, enabled: false }, sourceLabel: 'NDA Review' }
		});
		expect(screen.getByRole('link', { name: /edit/i })).toHaveAttribute(
			'href',
			'/automations/schedules/s1'
		);
...
<div class="flex items-center gap-3 rounded-mlq-control border border-mlq-subtle p-3">
	<div class="min-w-0">
		...
		<div class="truncate text-xs text-mlq-muted">
			{describeCron(schedule.cron_expr)} · {sourceLabel}
		</div>
	</div>

	<span class="ml-auto shrink-0 text-xs text-mlq-muted"
		>next: {formatWhen(schedule.next_run_at)}</span
	>
	...
	<form method="POST" action="?/toggle" use:enhance class="shrink-0">
		...
		<button
			...
			class="rounded-full px-2 py-0.5 text-xs font-medium {schedule.enabled
	...
	<a
		href="/automations/schedules/{schedule.id}"
...
{#if rows.length === 0}
	<div class="rounded-mlq-control border border-dashed border-mlq-subtle p-8 text-center">
		...
		<p class="mt-1 text-xs text-mlq-muted">
			A schedule runs a playbook or skill on a recurring cadence — handing a standing chore to
			Donna.
		</p>
		<ul class="mx-auto mt-3 max-w-md space-y-1 text-left text-xs text-mlq-muted">
			<li>
				• Drop documents into a knowledge base through the week, then generate a <strong
					>weekly summary document</strong
				> every Friday.
			</li>
			<li>
				• Regenerate a <strong>dashboard / digest document</strong> from your latest files on a cadence.
			</li>
...
export const actions: Actions = {
	create: async (event) => {
		...
		const res = await lqFetch(event, '/api/v1/autonomous/schedules', {
			method: 'POST',
	...
	toggle: async (event) => {
		...
		const res = await lqFetch(event, `/api/v1/autonomous/schedules/${id}`, {
			method: 'PATCH',
			body: JSON.stringify({ enabled })
		});
		if (!res.ok)
			return fail(res.status === 403 ? 403 : 502, { error: 'Could not update the schedule.' });
		return { toggled: true };
	},
	delete: async (event) => {
		const form = await event.request.formData();
		...
		if (!res.ok)
			return fail(res.status === 403 ? 403 : 502, { error: 'Could not delete the schedule.' });
		return { deleted: true };
	}
};
```
...
describe('/automations/schedules page', () => {
	it('shows the opt-in gate when autonomous is off', () => {
		render(Page, {
			props: { data: { autonomousEnabled: false, unread: 0, schedules: [], ...libs }, form: null }
		});
...
<script lang="ts">
	...
	const rows = $derived(
		data.schedules.map((s) => ({
			schedule: s,
			label: sourceLabel(s, data.playbookItems, data.skillItems)
		}))
	);
</script>
...
export const load: PageServerLoad = async (event) => {
	...
	const [schedulesRes, playbooksRes, userSkillsRes, builtinsRes, kbsRes, mattersRes] =
		await Promise.all([
			lqFetch(event, '/api/v1/autonomous/schedules'),
			lqFetch(event, '/api/v1/playbooks'),
			lqFetch(event, '/api/v1/user-skills?scope=user'),
...
<div class="mx-auto max-w-3xl px-4 py-6">
	...
	<a
		...
		class="mb-3 inline-block text-xs text-mlq-muted hover:text-mlq-text">← Schedules</a
	>
