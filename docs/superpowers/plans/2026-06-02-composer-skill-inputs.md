...
**Shared "provided" semantics** (used in the component, the controller, and gating): a value is _provided_ when — string: non-empty after trim; number: `Number.isFinite`; boolean: always (true/false); otherwise: `!= null`. `undefined` is never provided.
...
it('skillInputs is keyed by slug, coerced, and omits empty optionals + valueless skills', async () => {
	...
	const f2 = vi
		.fn()
		.mockResolvedValue(inputsRes([], [{ name: 'x', type: 'text', required: false }]));
...
describe('POST messages skill_inputs', () => {
	it('forwards skill_inputs when present', async () => {
		...
		await POST(
			event({ content: 'hi', model: 'smart', skill_inputs: { 'nda-review': { party: 'Acme' } } })
		);
 ...
 */
export function parseDraftSkillInputs(
	...
	try {
		...
		for (const [slug, vals] of Object.entries(parsed as Record<string, unknown>)) {
			if (vals && typeof vals === 'object' && !Array.isArray(vals))
				out[slug] = vals as Record<string, unknown>;
		}
		return out;
	} catch {
		return {};
...
<button
	type="button"
	onclick={submit}
	disabled={!value.trim() || !(skillAttach?.allRequiredFilled ?? true)}
	aria-label="Send"
...
if (data.draft && data.messages.length === 0)
	submit(data.draft, modelStore.selectedModel, data.draftSkills ?? [], data.draftSkillInputs ?? {});
```
...
test('composer: attach a skill, fill its required input, and send', async ({ page }) => {
	...
	await expect(page.locator('article, [data-role="assistant"]').last()).toBeVisible({
		timeout: 60_000
	});
});
```
