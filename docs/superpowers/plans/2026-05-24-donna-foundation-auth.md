...
@layer base {
  html { font-family: var(--font-serif); color: var(--color-mlq-text); background: var(--color-mlq-surface); }
}

/* Motion */
@keyframes shimmer { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }
@keyframes mlq-rise { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: none; } }
.mlq-rise { animation: mlq-rise 600ms cubic-bezier(0.22, 1, 0.36, 1) both; }
.mlq-rise-delay { animation: mlq-rise 600ms cubic-bezier(0.22, 1, 0.36, 1) 300ms both; }
```
...
describe('lqFetch', () => {
  ...
  it('refreshes once on 401, rotates cookies, retries', async () => {
    (fetch as any)
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ access_token: 'AT2', refresh_token: 'RT2', expires_in: 900 }), { status: 200 })) // refresh
...
export async function lqFetch(event: RequestEvent, path: string, init: RequestInit = {}): Promise<Response> {
  ...
  const tok = (await refreshed.json()) as { access_token: string; refresh_token: string; expires_in: number };
 ...
 * the stream request.
 */
export async function lqStream(event: RequestEvent, path: string, init: RequestInit = {}): Promise<Response> {
...
export const actions: Actions = {
  login: async (event) => {
    ...
    setSessionCookies(event, result.data.access_token, result.data.refresh_token, result.data.expires_in);
  ...
  mfa: async (event) => {
    ...
    if (result.kind !== 'ok') return fail(401, { mfa: true, mfaToken, error: 'Invalid code. Try again.' });
...
export const actions: Actions = {
  default: async (event) => {
    ...
    if (!ok) return fail(400, { error: 'Could not change password. Check your current password and policy.' });
...
<div class="mx-auto flex min-h-screen max-w-sm flex-col justify-center px-6">
  ...
  <p class="mb-6 text-center text-sm text-mlq-muted">Your account requires a password change before continuing.</p>
  <form method="POST" use:enhance class="space-y-4">
    ...
    <input name="new_password" type="password" autocomplete="new-password" placeholder="New password"
           class="w-full rounded-mlq-control border border-mlq-subtle px-3 py-2 outline-none" />
    <input name="confirm_password" type="password" autocomplete="new-password" placeholder="Confirm new password"
           class="w-full rounded-mlq-control border border-mlq-subtle px-3 py-2 outline-none" />
    {#if form?.error}<p class="text-sm text-mlq-error">{form.error}</p>{/if}
...
export default async function globalSetup() {
  ...
  if (!process.env.DONNA_E2E_EMAIL || !process.env.DONNA_E2E_PASSWORD) {
    throw new Error('Set DONNA_E2E_EMAIL and DONNA_E2E_PASSWORD (a bootstrapped lq-ai account) to run e2e.');
  }
