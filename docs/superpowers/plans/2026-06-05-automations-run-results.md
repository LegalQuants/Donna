...
describe('FindingCard', () => {
	...
	it('shows an unknown severity verbatim (lowercased, truncated) as a neutral badge', () => {
		render(FindingCard, {
			props: { finding: f({ severity: 'Needs-Partner-Review-Immediately-Long' }) }
		});
...
<div class="rounded-mlq-control border border-mlq-subtle p-3">
	<div class="mb-1 flex items-center gap-2">
		<span class="shrink-0 rounded-mlq-control px-1.5 py-0.5 text-[11px] {badgeClass}"
			>{badgeLabel}</span
		>
	...
	<p class="text-sm whitespace-pre-wrap text-mlq-text">{finding.content}</p>
</div>
```
...
describe('SessionDetail', () => {
	it('renders Results (from initial props) between the header and the timeline', () => {
		...
		expect(
			results.compareDocumentPosition(timelineHeading) & Node.DOCUMENT_POSITION_FOLLOWING
		).toBeTruthy();
	});
...
test('run-now → receipt shows the Results section after the run completes', async ({ page }) => {
	...
	await page
		...
		.fill('Adopt');
	await page
		.getByRole('button', { name: /Adopt a Pupper/ })
		.last()
		.click();
	...
	const hasFindings = await results
		.locator('text=/critical|warning|info/')
		.first()
		.isVisible()
		.catch(() => false);
	if (!hasFindings) {
