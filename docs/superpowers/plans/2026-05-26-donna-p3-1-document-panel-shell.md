...
async function seedCitedChat(): Promise<string> {
  ...
  const pid = await api(tok, '/projects', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'E2E DocPanel Matter' }) }).then((r) => r.json()).then((d) => d.id);
  const kid = await api(tok, '/knowledge-bases', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'E2E DocPanel KB' }) }).then((r) => r.json()).then((d) => d.id);
  ...
  const cid = await api(tok, '/chats', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title: 'E2E docpanel chat', project_id: pid }) }).then((r) => r.json()).then((d) => d.id);
...
test('clicking a verified citation opens the document panel and renders the PDF', async ({ page }) => {
