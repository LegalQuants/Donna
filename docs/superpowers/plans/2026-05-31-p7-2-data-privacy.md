...
vi.mock('$lib/server/session', () => ({
	clearSessionCookies: (...a: unknown[]) => clearSessionCookies(...a)
}));
...
describe('requestExport action', () => {
	it('POSTs and returns the job on 202', async () => {
		lqFetch.mockResolvedValue(
			new Response(JSON.stringify({ job_id: 'j1', status: 'queued', download_url: null }), {
				status: 202
			})
		);
	...
	it('maps a failure to a 502 inline error', async () => {
		...
		expect(r).toMatchObject({
			status: 502,
			data: { exportError: expect.stringMatching(/could not start/i) }
		});
	});
});
...
describe('requestDeletion action', () => {
	it('POSTs, clears session cookies, and returns the schedule on 202', async () => {
		lqFetch.mockResolvedValue(
			new Response(
				JSON.stringify({ scheduled_deletion_at: '2026-07-01T00:00:00Z', grace_period_days: 30 }),
				{ status: 202 }
			)
		);
		...
		expect(r).toEqual({
			deletion: { scheduled_deletion_at: '2026-07-01T00:00:00Z', grace_period_days: 30 }
		});
	});
	...
	it('maps a failure to a 502 and does NOT clear cookies', async () => {
		...
		expect(r).toMatchObject({
			status: 502,
			data: { deleteError: expect.stringMatching(/could not schedule/i) }
		});
	});
});
...
describe('cancelDeletion action', () => {
	...
	it('maps 400 (nothing pending) to a friendly message', async () => {
		...
		expect(r).toMatchObject({
			status: 400,
			data: { cancelMessage: expect.stringMatching(/no scheduled deletion/i) }
		});
	});
	...
	it('maps other failures to a 502 retry error', async () => {
		...
		expect(r).toMatchObject({
			status: 502,
			data: { cancelError: expect.stringMatching(/could not cancel/i) }
		});
	});
});
```
...
export const actions: Actions = {
	...
	requestDeletion: async (event) => {
		...
		if (!res.ok)
			return fail(502, { deleteError: 'Could not schedule deletion. Please try again.' });
...
<script lang="ts">
	...
	let {
		...
		open?: boolean;
		onclose?: () => void;
		ondeleted?: (info: DeletionSchedule) => void;
	} = $props();
	...
	const submit: SubmitFunction =
		...
		async ({ result }) => {
			if (result.type === 'success' && result.data?.deletion) {
				...
				error =
					(result.type === 'failure'
						? (result.data?.deleteError as string | undefined)
						: undefined) ?? 'Could not schedule deletion. Please try again.';
			}
...
{#if open}
	...
	>
		...
		<p class="mb-3 text-xs text-mlq-muted">
			This schedules permanent deletion after a grace period and signs you out everywhere. You can
		...
		<form method="POST" action="?/requestDeletion" use:enhance={submit}>
			...
			<div class="mt-4 flex justify-end gap-2">
				...
				<button
					type="submit"
					disabled={!canDelete}
					class="rounded-mlq-control bg-mlq-error px-2.5 py-1 text-xs text-white disabled:cursor-not-allowed disabled:opacity-50"
		...
		<div class="px-4 py-3 text-sm">
			...
			<div class="mt-4 border-t border-mlq-error/20 pt-3 text-xs text-mlq-muted">
				...
				<form method="POST" action="?/cancelDeletion" use:enhance={cancelSubmit} class="mt-1">
					<button type="submit" class="text-mlq-workflow hover:underline"
...
test('Data & privacy — export end-to-end, delete-confirm UI (no submit), cancel-nothing-pending', async ({
	page
}) => {
...
The running `donna-web` container serves _built_ code, so rebuild after adding `src/` files:
