...
it('forces attachment disposition so non-PDF bytes never render inline in our origin', async () => {
	lqFetch.mockResolvedValue(
		new Response(new Uint8Array([0x25]), { status: 200, headers: { 'content-type': 'text/html' } })
	);
	const res = await GET(event('f1'));
...
describe('UnsupportedFileCard', () => {
	...
	it('shows the exact mime type for transparency', () => {
		render(UnsupportedFileCard, {
			props: { fileId: 'f9', filename: 'terms.docx', mime: 'application/vnd.ms-excel' }
		});
	...
	it('derives the extension badge from the filename', () => {
		render(UnsupportedFileCard, {
			props: { fileId: 'f9', filename: 'report.final.csv', mime: 'text/csv' }
		});
	...
	it('falls back gracefully when filename is empty', () => {
		render(UnsupportedFileCard, {
			props: { fileId: 'f9', filename: '', mime: 'application/octet-stream' }
		});
...
<script lang="ts">
	...
	const ext = $derived(
		(filename.includes('.') ? (filename.split('.').pop() ?? '') : '').toUpperCase().slice(0, 5)
	);
</script>
...
<div class="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
	<div class="relative text-mlq-muted">
		...
		{#if ext}
			<span
				class="absolute inset-x-0 bottom-1 text-[8px] font-semibold tracking-wide text-mlq-muted"
				>{ext}</span
			>
		{/if}
	</div>
	<p class="max-w-full text-sm font-medium break-words text-mlq-text">{filename || 'Document'}</p>
...
it('renders a tab button per open document, marking the active one', () => {
	...
	const t2: DocTab = {
		...
		page: 2,
		quote: 'y',
		cite: { ...STUB_CITE, source_file_id: 'f2' },
		highlightStatus: 'pending'
	};
	render(DocumentPanel, {
		props: { docPanel: stub({ tabs: [t1, t2], activeId: 'f2', activeTab: t2 }) }
	});
...
```svelte
<div class="relative flex items-center gap-1 border-b border-mlq-subtle py-1.5 pr-1 pl-2">
	<div
	...
	<div class="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto">
		{#each docPanel.tabs as tab (tab.fileId)}
			<div
				class="flex max-w-[140px] shrink-0 items-center gap-0.5 rounded-mlq-control py-1 pr-0.5 pl-2 text-xs {tab.fileId ===
				docPanel.activeId
					? 'bg-mlq-surface-alt font-medium text-mlq-text'
					: 'text-mlq-muted hover:text-mlq-text'}"
			>
				<button
					type="button"
					...
					title={tab.filename || 'Document'}>{tab.filename || 'Document'}</button
				>
				<button
					type="button"
